...
# Function to adjust the playback speed of an audio file
...
st.title("PDF to Audio Converter with Real-Time Speed Control")
...
if uploaded_file is not None:
    ...
    if st.button("Convert to Audio"):
        ...
        if audio_path:
            ...
            speed = st.slider("Adjust Speed", min_value=0.5, max_value=2.0, step=0.1, value=1.0)
            
            # Adjust audio speed
            adjusted_audio = change_audio_speed(audio, speed)
            
            # Start playback in a background thread to allow real-time control
            if st.button("Play Audio"):
                threading.Thread(target=play_audio, args=(adjusted_audio,)).start()
